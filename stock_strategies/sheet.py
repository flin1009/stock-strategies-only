...
def write_chips_sell_streak(records: list[dict]):
    """整張 Chips_Sell_Streak 分頁清空重寫。"""
    sh = get_gsheet()
    try:
        ws = sh.worksheet("Chips_Sell_Streak")
        ws.clear()
    except gspread.WorksheetNotFound:
        rows_alloc = max(500, len(records) + 50)
        ws = sh.add_worksheet(
            title="Chips_Sell_Streak", rows=rows_alloc, cols=len(CHIPS_STREAK_HEADERS)
        )
